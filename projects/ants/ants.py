...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not place.is_hive:
            if place.bees and distance >= self.lower_bound and distance <= self.upper_bound:
...
# BEGIN Problem 6
class WallAnt(Ant):
    """WallAnt has high initial health but doing nothing each turn."""
...
class TankAnt(ContainerAnt):
    """TankAnts that can both protect the ant it contained and cause damage to bees."""
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 12
        super().action(gamestate)
        former_place = self.place.exit
        while former_place:
            if former_place.ant:
                if former_place.ant.is_container and former_place.ant.ant_contained:
                    former_place.ant.ant_contained.double()
                former_place.ant.double()
            former_place = former_place.exit
        # END Problem 12
