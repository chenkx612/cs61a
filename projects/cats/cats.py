...
def pick(paragraphs, select, k):
    ...
    ''
    """
    result = [string for string in paragraphs if select(string)]
    if k < len(result):
        return result[k]
    return ''
...
def about(subject):
    ...
    def f(string):
        words = lower(remove_punctuation(string)).split()
        for word in subject:
            if word in words:
                return True
        return False
...
def accuracy(typed, source):
    ...
    if len(source_words) == 0 and len(typed_words) == 0:
        return 100.0
    if len(source_words) == 0 or len(typed_words) == 0:
        return 0.0
    count = 0
...
def report_progress(typed, source, user_id, upload):
    ...
    while correct_words < len(typed):
        if typed[correct_words] == source[correct_words]:
            correct_words += 1
        else:
            break
    upload({'id': user_id, 'progress': correct_words / len(source)})
    return correct_words / len(source)
...
def time_per_word(words, timestamps_per_player):
    ...
    for i in range(len(tpp)):
        ...
        for j in range(len(tpp[i]) - 1):
            times_i.append(tpp[i][j+1]-tpp[i][j])
...
def fastest_words(words_and_times):
    ...
    for i in word_indices:
        time = [get_time(times, k, i) for k in player_indices]
        ans_list[time.index(min(time))].append(words[i])
    return ans_list
